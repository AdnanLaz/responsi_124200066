...
class BaseNetwork {
  ...
  static Future<List<dynamic>> getListData(String partUrl) async {
    final String fullUrl = baseUrl + "/" + partUrl;
    ...
    debugPrint("BaseNetwork - response : ${response.statusCode}");
    // debugPrint("BaseNetwork - response : ${response.body}");
    return _processResponseListData(response);
  }
  ...
  static Future<List<dynamic>> _processResponseListData(
      http.Response response) async {
    final body = response.body;
    if (body.isNotEmpty) {
      final jsonBody = json.decode(body);
